...
from fastpitch.NeMo.nemo.collections.common.parts.preprocessing import parsers
from fastpitch.NeMo.nemo.collections.tts.helpers.helpers import plot_alignment_to_numpy, plot_spectrogram_to_numpy
from fastpitch.NeMo.nemo.collections.tts.losses.aligner_loss import BinLoss, ForwardSumLoss
from fastpitch.NeMo.nemo.collections.tts.losses.fastpitchloss import DurationLoss, MelLoss, PitchLoss
from fastpitch.NeMo.nemo.collections.tts.models.base import SpectrogramGenerator
from fastpitch.NeMo.nemo.collections.tts.modules.fastpitch import FastPitchModule
from fastpitch.NeMo.nemo.collections.tts.torch.tts_data_types import SpeakerID
from fastpitch.NeMo.nemo.core.classes import Exportable
from fastpitch.NeMo.nemo.core.classes.common import PretrainedModelInfo, typecheck
from fastpitch.NeMo.nemo.core.neural_types.elements import (
    Index,
    LengthsType,
    MelSpectrogramType,
    ProbsType,
...
from fastpitch.NeMo.nemo.core.neural_types.neural_type import NeuralType
from fastpitch.NeMo.nemo.utils import logging, model_utils
...
class FastPitchModel(SpectrogramGenerator, Exportable):
    ...
    def __init__(self, cfg: DictConfig, trainer: Trainer = None):
        ...
        speaker_emb_condition_prosody = cfg.get(
            "speaker_emb_condition_prosody", False)
        speaker_emb_condition_decoder = cfg.get(
            "speaker_emb_condition_decoder", False)
        speaker_emb_condition_aligner = cfg.get(
            "speaker_emb_condition_aligner", False)
        ...
        self.export_config = {"enable_volume": False,
    ...
    def _setup_normalizer(self, cfg):
        if "text_normalizer" in cfg:
            ...
            try:
                self.normalizer = instantiate(
                    cfg.text_normalizer, **normalizer_kwargs)
    ...
    def parse(self, str_input: str, normalize=True) -> torch.tensor:
        if self.training:
            logging.warning("parse() is meant to be called in eval mode. AAAA")

        if normalize and self.text_normalizer_call is not None:
            str_input = self.text_normalizer_call(
                str_input, **self.text_normalizer_call_kwargs)
        print(f"Normalized: {str_input}")
        if self.learn_alignment:
    ...
    def forward(
        ...
        *,
        ...
        logging.warning("Inside forward")
    ...
    def generate_spectrogram(
        ...
        spect, *_ = self(text=tokens, durs=None, pitch=None,
    ...
    def training_step(self, batch, batch_idx):
        ...
        if self.learn_alignment:
            ...
            bin_loss_weight = min(self.current_epoch /
                                  self.bin_loss_warmup_epochs, 1.0) * 1.0
            bin_loss = self.bin_loss(
                hard_attention=attn_hard, soft_attention=attn_soft) * bin_loss_weight
    ...
    def validation_step(self, batch, batch_idx):
        ...
        mels, mel_lens = self.preprocessor(
            input_signal=audio, length=audio_lens)
    ...
    def validation_epoch_end(self, outputs):
        def collect(key): return torch.stack([x[key] for x in outputs]).mean()
        ...
        if cfg.dataset._target_ == "nemo.collections.tts.torch.data.TTSDataset":
            ...
            if hasattr(self.vocab, "set_phone_prob"):
                phon_mode = self.vocab.set_phone_prob(
                    prob=None if name == "val" else self.vocab.phoneme_probability)
    ...
    def input_example(self, max_batch=1, max_dim=44):
        ...
        sz = (max_batch * max_dim,
              ) if self.export_config["enable_ragged_batches"] else (max_batch, max_dim)
        inp = torch.randint(
        ...
        pace = torch.clamp(torch.randn(sz, device=par.device,
                           dtype=torch.float32) * 0.1 + 1, min=0.01)

        inputs = {'text': inp, 'pitch': pitch, 'pace': pace}

        if self.export_config["enable_volume"]:
            volume = torch.clamp(torch.randn(
                sz, device=par.device, dtype=torch.float32) * 0.1 + 1, min=0.01)
            inputs['volume'] = volume
        if self.export_config["enable_ragged_batches"]:
            batch_lengths = torch.zeros(
                (max_batch + 1), device=par.device, dtype=torch.int32)
            left_over_size = sz[0]
            batch_lengths[0] = 0
            for i in range(1, max_batch):
                length = torch.randint(
                    1, left_over_size - (max_batch - i), (1,), device=par.device)
    ...
    def interpolate_speaker(
        ...
        speaker_emb_1 = (
            self.fastpitch.speaker_emb(torch.tensor(
                original_speaker_1, dtype=torch.int32).cuda()).clone().detach()
        )
        speaker_emb_2 = (
            self.fastpitch.speaker_emb(torch.tensor(
                original_speaker_2, dtype=torch.int32).cuda()).clone().detach()
        )
        new_speaker_emb = weight_speaker_1 * \
            speaker_emb_1 + weight_speaker_2 * speaker_emb_2
...
def create_batch(
    ...
    index = 1
    texts = torch.zeros(
        batch_lengths.shape[0] - 1, max_len, dtype=torch.int64, device=text.device) + padding_idx
    pitches = torch.zeros(
        batch_lengths.shape[0] - 1, max_len, dtype=torch.float32, device=text.device)
    paces = torch.zeros(
        batch_lengths.shape[0] - 1, max_len, dtype=torch.float32, device=text.device) + 1.0
    volumes = torch.zeros(
        batch_lengths.shape[0] - 1, max_len, dtype=torch.float32, device=text.device) + 1.0
