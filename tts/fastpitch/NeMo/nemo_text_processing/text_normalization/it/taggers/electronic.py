...
class ElectronicFst(GraphFst):
    ...
    def __init__(self, deterministic: bool = True):
        ...
        accepted_symbols = pynini.project(pynini.string_file(
            get_abs_path("data/electronic/symbol.tsv")), "input")
        accepted_common_domains = pynini.project(
            pynini.string_file(get_abs_path(
                "data/electronic/domain.tsv")), "input"
        )
        all_accepted_symbols = NEMO_ALPHA + \
        ...
        username = (
            pynutil.insert("username: \"") + all_accepted_symbols +
            pynutil.insert("\"") + pynini.cross('@', ' ')
        )
        domain_graph = all_accepted_symbols + \
            pynini.accep('.') + all_accepted_symbols + NEMO_ALPHA
        protocol_symbols = pynini.closure(
            (graph_symbols | pynini.cross(":", "due punti")) + pynutil.insert(" "))
        protocol_start = (pynini.cross("https", "acca ti ti pi esse ") | pynini.cross("http", "acca ti ti pi ")) + (
            pynini.accep("://") @ protocol_symbols
        )
        protocol_file_start = pynini.accep(
            "file") + insert_space + (pynini.accep(":///") @ protocol_symbols)

        protocol_end = pynini.cross(
            "www", "vu vu vu punto ") + pynutil.delete(".") @ protocol_symbols
        ...
        domain_common_graph = (
            ...
            + pynini.difference(
                ...
                + pynini.closure(accepted_symbols + pynini.closure(NEMO_ALPHA |
                                 NEMO_DIGIT | accepted_symbols), 0, 1),
                pynini.project(protocol, "input") + NEMO_SIGMA,
            )
            + pynutil.insert("\"")
        )

        protocol = pynutil.insert("protocol: \"") + \
            protocol + pynutil.insert("\"")
        # email
