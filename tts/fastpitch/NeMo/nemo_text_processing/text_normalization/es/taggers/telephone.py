...
import pynini
from NeMo.nemo_text_processing.text_normalization.en.graph_utils import NEMO_SIGMA, GraphFst, insert_space
from NeMo.nemo_text_processing.text_normalization.es.graph_utils import ones
...
class TelephoneFst(GraphFst):
    ...
    def __init__(self, deterministic: bool = True):
        ...
        single_digits = pynini.invert(graph_digit).optimize() | pynini.cross("0", "zero")

        double_digits = pynini.union(
            graph_twenties,
            graph_teen,
            (graph_ties + pynutil.delete("0")),
            (graph_ties + insert_space + pynutil.insert("e") + insert_space + graph_digit),
        )
